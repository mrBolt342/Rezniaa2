...
namespace JUTPS.UI
{
    public class UIInteractMessages : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            // >> Vehicle Message
            if(JUGameManager.PlayerController.VehicleInArea != null && JUGameManager.PlayerController.IsDriving == false)
            {
                PickUpMessageObject.SetActive(true);
                UIElementToWorldPosition.SetUIWorldPosition(PickUpMessageObject, JUGameManager.PlayerController.VehicleInArea.transform.position, VehicleOffset);
                if (WarningText)
                {
