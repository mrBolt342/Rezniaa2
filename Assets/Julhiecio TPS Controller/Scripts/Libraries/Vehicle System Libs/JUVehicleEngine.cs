...
namespace JUTPS.VehicleSystem
{
	...
	public class Vehicle : MonoBehaviour
	{
		...
		private void UpdateWheels()
		{
			...
			float brake = Mathf.Abs(_throttle) < INPUTS_DEATH_ZONE ? Engine.AutoBrakeForce : brakeForce * FinalBrake;

			if (!IsOn)
			{
				for (int i = 0; i < WheelsData.Length; i++)
				{
					WheelsData[i].ApplyControl(0, 0, brake);
			...
			float throttle = engineTorque * FinalThrottle;
			float steer = FinalSteer * CurrentSteerVsSpeed;
