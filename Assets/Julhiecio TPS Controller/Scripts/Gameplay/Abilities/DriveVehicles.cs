    ...
    public class DriveVehicles : JUTPSActions.JUTPSAction
    {
        ...
        protected virtual void DrivingLocomotion()
        {
            ...
            if (rb != null)
            {
                ...
                rb.useGravity = false;
                //Simulate Inert
                rb.velocity = VehicleToDrive.RigidBody.velocity;
            }

            if (VehicleToDrive.InverseKinematicTargetPositions.CharacterPosition)
            {
                transform.position = VehicleToDrive.InverseKinematicTargetPositions.CharacterPosition.position;
                transform.rotation = VehicleToDrive.InverseKinematicTargetPositions.CharacterPosition.rotation;
            }
        }
        ...
        public void ExitVehicle()
        {
            ...
            if (IsDriving == true)
            {
                if (TPSCharacter != null)
                {
                    ...
                    TPSCharacter.transform.position = VehicleToDrive.GetExitPosition(GroundLayer);
                }
                else
                {
                    transform.position = VehicleToDrive.GetExitPosition(GroundLayer);
                }
                OnExitVehicle();
            }
            //Turn On Physics
